...
class WanVideoNeoVersePipeline(BasePipeline):
    ...
    def load_lora(self, module, path=None, state_dict=None, alpha=1, lora_type="diffsynth"):
        if lora_type != "diffsynth":
            raise ValueError(f"Unsupported lora_type {lora_type}. Only 'diffsynth' is supported in the new pipeline.")
    ...
    def from_pretrained(
        ...
        # 3) Use WanVideoPipeline.from_pretrained to load the WAN side (dit / text_encoder / vae / tokenizer).
        base_model_configs = []
        if load_dit:
        ...
        pipe.text_encoder = base_pipe.text_encoder if load_text_encoder else None
        pipe.tokenizer = base_pipe.tokenizer
        pipe.dit = base_pipe.dit if load_dit else None
        ...
        if lora_path is not None:
            ...
            pipe.load_lora(pipe.dit, lora_path, alpha=lora_alpha, lora_type="diffsynth")
