...
from models.control_latent.geometry import (
    crop_image_depthmap,
...
class EasyDataset:
    """Minimal dataset base used by `BaseDataset`."""
...
class BaseDataset(EasyDataset):
    ...
    def __getitem__(self, idx):
        ...
        for v in range(len(views)):
            ...
            if v > 0 and view["is_target"] == views[v - 1]["is_target"] and not allow_nonmonotonic:
                assert view["timestamp"] >= views[v - 1]["timestamp"], (
                    f"Timestamp for view {view_name(view)} is not greater than or equal to previous view."
                )
            else:
    ...
    def _crop_resize_if_necessary(
        ...
        if self.aug_crop > 1:
            ...
            delta_target_ratio = rng.random() * (1.0 / self.aug_crop - 1.0)
