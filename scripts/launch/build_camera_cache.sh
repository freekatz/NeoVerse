...
if [[ "${DRY_RUN}" == "1" ]]; then
  log "+ bash -n $0"
  log "+ ${ENV_PYTHON} -m py_compile tools/cache/build_camera_cache.py training/data/datasets/spatialvid.py"
else
  bash -n "$0"
  "${ENV_PYTHON}" -m py_compile tools/cache/build_camera_cache.py training/data/datasets/spatialvid.py
fi
