...
#   GPU_LIST=0,1,2,3 bash scripts/build_spatialvid_frozen_cache.sh
...
if [[ "${DRY_RUN}" == "1" ]]; then
  log "+ bash -n $0"
  log "+ ${ENV_PYTHON} -m py_compile experiments/build_spatialvid_frozen_cache.py train_distill_control_latent.py training/data/datasets/spatialvid.py training/data/temporal_trajectory.py"
else
  bash -n "$0"
...
for shard_index in "${!gpu_ids[@]}"; do
  ...
  shard_cmd=(
    "${ENV_PYTHON}" experiments/build_spatialvid_frozen_cache.py
    "${common_args[@]}"
